...
function getFilteredServices() {
	...
	foreach ($services as $service) {
		...
		echo 	"<div class='readMoreh'>";
		echo 		"<a href='" . get_permalink($service->ID) . "' class='dropbtn'>" . pll__('Read more');
...
function getFilteredEvents() {
	...
	if (isset($postMeta['eventbrite'])) {
		...
		$eventbriteMeta = unserialize($postMeta['eventbrite'][0]);
			...
			$startString = get_post_meta(get_the_ID())['_event_start_date'][0];
			...
			$endString = get_post_meta(get_the_ID())['_event_end_date'][0];
	        ...
	        echo "<div class='eventInfoGrid'>";
	        echo    "<div class='datetime'>";
	        echo        "<h5>" . $startDate . "</h5>";
	        echo        "<p>" . $startTime . "<p>";
	        echo    "</div>";
	        echo    "<div class='titleInfo'>";
	        echo        "<h5 style='text-align: left'>" . get_the_title() . "</h5>";
	        echo        "<p style='text-align: left'>" . wp_trim_words(get_the_content(), 10) . "</p>";
	        echo 	"</div>";
	        echo "<div class='location' style='text-align: right; width: 250px'>";
			echo	"<h5 style='text-align: end'>Event Location</h5>";
			if ($isEventbrite) {
			echo 			"<p style='text-align: end'>" . $eventbriteName . "</p>";
			echo			"<p style='text-align: end'>" . $eventbriteLocation[0] . "</p>";
			echo			"<p style='text-align: end'>" . $eventbriteLocation[1] . "</p>";
			}else{
			echo 			"<p style='text-align: end'>" . em_get_locations_for(get_the_ID())[0]->name . "</p>";
			echo			"<p class='a1' style='text-align: end'>" . (!empty($eventLocation['address']) ? $eventLocation['address'] : '') . "</p>";
			echo			"<p class='a2' style='text-align: end'>" . (!empty($eventLocation['zip']) ? $eventLocation['zip'] : '') . "</p>";
			echo 			"<p class='a3' style='text-align: end'>" . (!empty($eventLocation['city']) ? $eventLocation['city'] : '') . "</p>";
			}
			echo	"</div>";
			echo    "<div class='readMore' style='text-align: left'>";
	        echo        "<a href='" . get_the_permalink() . "' class='dropbtn bold'>" . pll__('Read more');
	        echo "<img class='cardArrow' src='" . get_stylesheet_directory_uri() . "/Icons/keyboard_arrow_right-24px.svg' alt='placeholder icon'>";
	        echo        "</a>";
	        echo    "</div>";
	        echo    "</div>";
    ...
    if ($events->found_posts - $eventsOffset > 3) {
    	echo "<a href='javascript:void(0)' id='loadMoreEvents' data-offset='" . ($eventsOffset + 3) . "'>" . pll__('Load more') . "</a>";
    }
...
function getFilteredNews() {
		...
		while($news->have_posts()) {
			...
			echo "<div class='News'>";
	            ...
	            echo "<div class='newsText'>";
	                ...
	                echo "<h4>" . wp_trim_words(get_the_title(), 4) . "</h4>";
	                ...
	                echo "<div class='readMore' style='font-weight: 600;'>";
	                    echo "<a href='" . get_the_permalink() . "' class='dropbtn'>" . pll__('Read more');
	                        echo "<img class='cardArrow' src='" . get_stylesheet_directory_uri() . "/Icons/keyboard_arrow_right-24px.svg' alt='placeholder icon'>";
	                    echo "</a>";
	               echo " </div>";
	           echo " </div>";
    ...
    if ($news->found_posts - $newsOffset > $newsLimit) {
    	echo "</div>"; //closes previous div (flex)
    	if ($loadMoreButton) {
    		echo "<a class='tietoaMeista_link' style='margin: 30px auto 0px auto' href='javascript:void(0)' id='loadMoreNews' data-offset='" . ($newsOffset + $newsLimit) . "'>" . pll__('Load more') ."</a>";
    	}
    }
...
add_action('init', function() {
	...
	pll_register_string('NewCo-helsinki_Event-page', 'Link to event');
	pll_register_string('NewCo-helsinki_Copyright', 'Copyright');
	pll_register_string('NewCo-helsinki_Events', 'Events');
	pll_register_string('NewCo-helsinki_FliterEvents', 'Filter events');
});
